...
[![Windows 一键运行](https://img.shields.io/badge/Windows-一键本地运行-0A64BC?logo=windows11&logoColor=white)](https://github.com/Liang-07-wen/tomato-insight/releases/latest/download/Tomato-Insight-Windows.zip)

[Windows 一键运行](#windows-一键本地运行) · [项目展示](#项目展示) · [手动运行](#手动本地运行) · [接口说明](docs/API.md) · [版本记录](CHANGELOG.md)
</div>
...
项目提供 Windows 一键本地运行和手动运行两种方式，可在本机浏览器中使用叶片与果实检测功能。

## Windows 一键本地运行

点击下方按钮下载 GitHub Release 中的 Windows 完整包：

[![下载 Windows 一键运行完整包](https://img.shields.io/badge/下载-Tomato--Insight--Windows.zip-2EA44F?style=for-the-badge&logo=github)](https://github.com/Liang-07-wen/tomato-insight/releases/latest/download/Tomato-Insight-Windows.zip)

使用步骤：

1. 下载并完整解压 `Tomato-Insight-Windows.zip`。
2. 双击 `TomatoInsightLauncher.exe`。
3. 首次运行会自动检测或安装 Python、创建 `.venv` 并安装项目依赖。
4. Flask 服务启动成功后，程序会自动在默认浏览器中打开网站。
5. 网站运行期间保留启动器窗口，关闭窗口即可结束本地服务。

> 请使用 GitHub Release 完整包。GitHub 源码 ZIP 中的 ONNX 文件可能只是 Git LFS 指针，不适合作为完整运行包。

## 项目展示

### 首页

![Tomato Insight 首页](docs/images/showcase/home.png)

### 叶片病害检测

![Tomato Insight 叶片病害检测结果](docs/images/showcase/leaf-detection.png)

### 果实病害检测

![Tomato Insight 果实病害检测结果](docs/images/showcase/fruit-detection.png)

<table>
  <tr>
    <td width="50%">
      <strong>项目介绍</strong><br>
      <img src="docs/images/showcase/about.png" alt="Tomato Insight 项目介绍">
    </td>
    <td width="50%">
      <strong>关于我们</strong><br>
      <img src="docs/images/showcase/developer.png" alt="Tomato Insight 关于我们">
    </td>
  </tr>
</table>
...
| 运行方式 | Windows 一键启动器、Flask 本地服务、Docker |
| 模型管理 | Git LFS |
...
├─ TomatoInsightLauncher.exe  # Windows 一键本地启动器
├─ launcher/                  # Windows 启动器源代码
├─ Dockerfile                 # Docker 运行镜像
...
- 已完成本地功能验收、Windows 一键运行、Docker 配置，以及桌面端、移动端页面适配。
